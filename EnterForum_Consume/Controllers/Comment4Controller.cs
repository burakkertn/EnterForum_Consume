...
{
    public class Comment4Controller : Controller
    {
        private readonly IHttpClientFactory _httpClientFactory;
        public Comment4Controller(IHttpClientFactory httpClientFactory)
        {
        ...
        {
            int id = (int)TempData["idForCommentWorld"];
        ...
        var responseMessage = await client.PostAsync("http://localhost:5074/api/Comment", content);
            ...
            {
                return RedirectToAction("Detail1",$"{controllerName}");
    }
            return View();
}
    }
