...
namespace EnterForum_Consume.ViewComponents.Technology
{
    public class _TechnologyTopicTitles : ViewComponent
    {
        private readonly IHttpClientFactory _httpClientFactory;

        public _TechnologyTopicTitles(IHttpClientFactory httpClientFactory)
        {
        ...
        public async Task<IViewComponentResult> InvokeAsync()
        {
            ...
            if (responseMessage.IsSuccessStatusCode)
            {
                ...
                var filteredValues = values.Where(x => x.CategoryID == 1).ToList();
