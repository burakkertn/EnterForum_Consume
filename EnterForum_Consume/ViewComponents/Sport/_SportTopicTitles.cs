...
namespace EnterForum_Consume.ViewComponents.Sport

{
    public class _SportTopicTitles : ViewComponent
    {
        private readonly IHttpClientFactory _httpClientFactory;

        public _SportTopicTitles(IHttpClientFactory httpClientFactory)
        {
        ...
        public async Task<IViewComponentResult> InvokeAsync()
        {
            ...
            if (responseMessage.IsSuccessStatusCode)
            {
                ...
                var filteredValues = values.Where(x => x.CategoryID == 2).ToList();
