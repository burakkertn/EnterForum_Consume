...
namespace EnterForum_Consume.ViewComponents.Shop

{
    public class _ShopTopicTitles : ViewComponent
    {
        private readonly IHttpClientFactory _httpClientFactory;

        public _ShopTopicTitles(IHttpClientFactory httpClientFactory)
        {
        ...
        public async Task<IViewComponentResult> InvokeAsync()
        {
            ...
            if (responseMessage.IsSuccessStatusCode)
            {
                ...
                var filteredValues = values.Where(x => x.CategoryID == 4).ToList();
